...
1. **任务次数限制**: 代码注释模式默认10个任务，最大支持20个任务；代码补全模式默认2000个任务
2. **文件数量限制**: 单次最多加载20个源文件（自动随机选择）
3. **文件编码**: 文件必须是 UTF-8 编码
4. **空文件跳过**: 自动跳过空文件或读取失败的文件
5. **随机选择**: 每次请求随机选择一个文件进行注释生成
6. **流式输出**: 代码注释生成采用流式输出，需等待 `isEnd=1` 才算完成
7. **超限保护**: 如果输入超过20个任务，系统会自动调整为20
